...
def get_elections():
    soup = make_soup("https://en.wikipedia.org/wiki/List_of_United_Kingdom_general_elections")
...
def get_population():
    population = pd.read_excel(config.SOURCE_DIR+r"\ons\population\a33197de.xls", "UK Population", names=['Blank','Year','TotalPopulation','AnnualGrowthRatePerc','AnnualGrowth'])
    ...
    population['TotalPopulation'] = pd.to_numeric(population['TotalPopulation'], errors='coerce')
...
def get_results():
    if use_local:
        #print use_local
        return pd.read_csv(config.MASTER_FILES["pop_results"], encoding='utf-8', sep='|')
    else:
        pieces = []
        for index, row in get_elections().iterrows():
            print "Fetching results for "+str(row['Year'])+"..."
            soup = make_soup("https://en.wikipedia.org/wiki/United_Kingdom_general_election,_"+str(row['Year']))
            
            if row['Year'] <= 1979:
                table = soup.find('caption', text="UK General Election "+str(row['Year'])).find_parent("table")
            elif row['Year'] <= 1983:
                table = soup.find('caption', text="United Kingdom General Election "+str(row['Year'])).find_parent("table")
            elif row['Year'] <= 2001:
                table = soup.find('table', attrs={"class": "wikitable"})
            else:
                table = soup.find('table', attrs={"class": "wikitable sortable", "style": "text-align:right; font-size: 95%;"})
            rows = table.findAll('tr')
            ...
            for r in rows:
                ...
                if ((len(data_row) > 10) and (data_row[1] not in ('Political party','Leader'))):
                    if row['Year'] <= 2001:
            ...
            dataframe = pd.DataFrame(data,columns=['Political party', 'Leader', 'Candidates', 'Elected', 'Votes'])
            ...
            for col in ['Candidates', 'Elected', 'Votes']:
                #print dataframe[col]
                dataframe[col] = pd.to_numeric(dataframe[col].astype(str).str.replace(',',''), errors='coerce')
            #dataframe.info()
            #print dataframe.describe(include="all")
            #print dataframe[:4]
        ...
        results["Political party"] = results["Political party"].str.replace(r"\[.*\]","")
...
def show_dfplot(dftable, ylabel, ymax, yinterval, dfplotfile):
    ...
    dfplot.set_xticks( range(min_year, max_year), minor=True )
    dfplot.set_yticks( range(0, ymax, yinterval), minor=True )
...
def do_analysis():
    ...
    if use_local:
        fulldata.drop(['Political party', 'Leader', 'Candidates', 'Elected', 'AnnualGrowth', 'AnnualGrowthRatePerc', 'TotalPopulation', 'Votes', 'Unnamed: 0'], axis=1, inplace=True)
    else:
        fulldata.drop(['Political party', 'Leader', 'Candidates', 'Elected', 'AnnualGrowth', 'AnnualGrowthRatePerc', 'TotalPopulation', 'Votes'], axis=1, inplace=True)
    ...
    pop_table.loc[len(pop_table.index)] = np.array([2016, 0, 65572409, 16141241, 17410742, 33551983])
    
    #manually add 2017 election - FOR NOW
    pop_table.loc[len(pop_table.index)] = np.array([2017, 0, 66029928, (12874985+2371772+977569+525371+164466), (13632914+593852+4642), ((12874985+2371772+977569+525371+164466)+(13632914+593852+4642))])
