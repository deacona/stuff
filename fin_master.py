...
def get_legacy_data():
    df = pd.read_csv(
            ...
            header=0,
            names=["Date","Description","Credit_amount","Debit_amount","BalanceText","Year","Month","YearMonth","Debit_Credit_Flag","Amount","Balance","Account","Type","Category"],
            )
    ...
    df['Debit_Credit_Flag'], df['Amount'] = zip(*df['Transaction'].map(func_debit_credit))
    ...
    df['Description_Item'] = df['Description_Clean']+' // '+df['Type']+' // '+df['Debit_Credit_Flag']
...
def get_categories():
    ...
    cat = pd.DataFrame(df.groupby('Description_Clean')['Category'].agg(lambda x: x.value_counts().index[0]))
    cat = cat.reset_index()
...
def get_statements(filename):
    ...
    df[["Account","Amount","Balance","Date","Description"]] = df[["Account","Amount","Balance","Date","Description"]].apply(clean_text)
...
def data_validator(df):
    ...
    checks = df[["Account_Holder","Filename","Date","Transaction","Balance","checks"]][(df["checks"] >= 1) & (df["Account_Holder"].shift(1) == df["Account_Holder"]) & (df["Filename"] <> "fin_legacy.csv")]
...
def get_account_data():
    ...
    cols=["Filename","Lineno","Account_Holder","Bank","Date","Year","Month","Day_Of_Month","Day_Of_Week","YearMonth","DateKey","Day_Abrrev","Weekday_Flag","Week_Begin_Date","Month_Abbrev","Quarter","Last_Day_In_Month_Flag","Same_Day_Year_Ago","Type","Description","Description_Clean","Description_Item","Category","Merchant","Place","Category_Group","Debit_Credit_Flag","Transaction","Amount","Balance"]
    df = df[cols]
...
#    plt.show()
    ...
    print dataset[dataset["Predicted_Category_Group"] != dataset["Category_Group"]].head(1)
...
def get_new_files():
    for filename in os.listdir(config.DOWNLOAD_DIR):
        ...
        if filename.startswith(tuple(account_dict.keys())):
            for prefix in account_dict.keys():
...
def archive_files():
    ...
    for zipprefix, zipfolder in finances_dict.iteritems():
        ...
        rc = subprocess.Popen(cmd, executable=zProg, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
...
def update_budget_planner(df):
    ...
    sheet.cell(row=20, column=5).value = inputs["i01. Income From Employment / Self Employment"]
    sheet.cell(row=21, column=5).value = inputs["i02. Income From Savings & Investments"]
...
def export_viz(df):
    ...
    #df.info()
    bal = df[df["Account_Holder"] == "Joint"].groupby("DateKey").agg({'Balance': [min, max, np.mean, np.median]})
